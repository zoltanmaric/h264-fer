...
void MotionCompensateSubMBPart(frame *current, frame *refPic,
  ...
  for(y=0; y<4; ++y)
    for(x=0; x<4; ++x)
      current->Luma[(y+org_y)*FRAME_Width+x+org_x] = L_MC_frac_interpol(&(L_Temp_4x4_refPart[y+2][x+2]), frac);
  ...
  for(int iCbCr=0; iCbCr<2; ++iCbCr) {
    ...
    for(y=0; y<2; ++y)
      for(x=0; x<2; ++x)
		current->Chroma[iCbCr][(y+org_y)*FRAME_Width + x+org_x] =
					  ((8-xLinear)*(8-yLinear) * C_Temp_4x4_refPart[iCbCr][y][x]  +
						  xLinear *(8-yLinear) * C_Temp_4x4_refPart[iCbCr][y][x+1]+
					   (8-xLinear)*   yLinear  * C_Temp_4x4_refPart[iCbCr][y+1][x]  +
						  xLinear *   yLinear  * C_Temp_4x4_refPart[iCbCr][y+1][x+1]+
