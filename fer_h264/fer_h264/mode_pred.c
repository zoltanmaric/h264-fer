...
void MPI_Init()
{
	infos->mb_type =  malloc(FR_numRows*FR_MB_in_row*sizeof(int));
	infos->mvL0x =  malloc(16*FR_numRows*FR_MB_in_row*sizeof(int)); // max 16 different motion vectors per macroblock
	infos->mvL0y =  malloc(16*FR_numRows*FR_MB_in_row*sizeof(int));
	//infos->mvCL0x = malloc(16*FR_numRows*FR_MB_in_row*sizeof(int)); // equal to mvL0x
	//infos->mvCL0y = malloc(16*FR_numRows*FR_MB_in_row*sizeof(int)); // equal to mvL0y
	infos->subMvCnt = malloc(FR_numRows*FR_MB_in_row*sizeof(int));
	infos->refIdxL0 = malloc(FR_numRows*FR_MB_in_row*sizeof(int));
	MPI_Clear();
}
...
bool get_neighbour_mv(int org_x, int org_y, int mbPartIdx, int curr_refIdxL0, int * mvNx, int * mvNy, int * refIdxL0N)
{
	*mvNx = MV_NA; *mvNy = MV_NA;
	...
	if (   MPI_mb_type(org_x, org_y) != P_L0_16x16
		...
		|| MPI_refIdxL0(org_x, org_y) != currRefIdxL0) 
	{
...
void DeriveMVs(int org_x, int org_y, int mvdx, int mvdy) {
	...
	if (MPI_subMvCnt(org_x, org_y) == 2)
	{
		if (MbPartWidth(*(infos->mb_type + mbPardIdx)) == 16) // P_16x8
		{
