...
#define MPI_mb_type(org_x,org_y) *(infos->mb_type+((org_y/MB_Height)*FR_MB_in_row+org_x/MB_Width))
#define MPI_mvL0x(org_x,org_y,subMbIdx) *(infos->mvL0x+16*((org_y/MB_Height)*FR_MB_in_row+org_x/MB_Width)+subMbIdx*4)
#define MPI_mvL0y(org_x,org_y,subMbIdx) *(infos->mvL0y+16*((org_y/MB_Height)*FR_MB_in_row+org_x/MB_Width)+subMbIdx*4)
#define MPI_mvSubL0x_byIdx(mbPartIdx,subMbIdx,subMbPartIdx) *(infos->mvL0x+16*mbPartIdx+4*subMbIdx+subMbPartIdx)
#define MPI_mvSubL0y_byIdx(mbPartIdx,subMbIdx,subMbPartIdx) *(infos->mvL0y+16*mbPartIdx+4*subMbIdx+subMbPartIdx)
#define MPI_mvSubL0x(org_x,org_y,subMbIdx,subMbPartIdx) *(infos->mvL0x+16*((org_y/MB_Height)*FR_MB_in_row+org_x/MB_Width)+subMbIdx*4+subMbPartIdx)
#define MPI_mvSubL0y(org_x,org_y,subMbIdx,subMbPartIdx) *(infos->mvL0y+16*((org_y/MB_Height)*FR_MB_in_row+org_x/MB_Width)+subMbIdx*4+subMbPartIdx)
#define MPI_mvCL0x(org_x,org_y,subMbIdx) *(infos->mvL0x+16*((org_y/MB_Height)*FR_MB_in_row+org_x/MB_Width)+subMbIdx*4)
#define MPI_mvCL0y(org_x,org_y,subMbIdx) *(infos->mvL0y+16*((org_y/MB_Height)*FR_MB_in_row+org_x/MB_Width)+subMbIdx*4)
#define MPI_subMvCnt(org_x,org_y) *(infos->subMvCnt+((org_y/MB_Height)*FR_MB_in_row+org_x/MB_Width))
#define MPI_refIdxL0(org_x,org_y) *(infos->refIdxL0+((org_y/MB_Height)*FR_MB_in_row+org_x/MB_Width))

typedef struct _MB_pred_info {
  int * mb_type; // type of current macroblock
  int * mvL0x,* mvL0y; // luma motion vectors, mvL1='na' in baseline, pointer to mv of first subMB in current block
  //int * mvCL0x,* mvCL0y; // chroma motion vectors, in baseline, no field macroblock (every block is frame block), so mvCL0=mvL0
  int * subMvCnt; // total number of motion vectors used, postaviti na NumMbPart(mb_type)
  int * refIdxL0;//, * refIdxL1; // reference pictures, refIdxL1='na' in baseline
  //bool * predFlag0, * predFlag1; // irrelevant as predFlag0=1 and predFlag1=0 in baseline profile
} MB_pred_info;
...
bool get_neighbour_mv(int org_x, int org_y, int mbPartIdx, int curr_refIdxL0, int * mvNx, int * mvNy, int * refIdxL0N)
{
	*mvNx = MV_NA; *mvNy = MV_NA;
	...
	if (   MPI_mb_type(org_x, org_y) != P_L0_16x16
		...
		&& MPI_mb_type(org_x, org_y) != P_L0_16x8
		&& MPI_mb_type(org_x, org_y) != P_L0_8x16
